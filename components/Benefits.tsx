    ...
    <section className="py-24 px-6 bg-spinecho-dark border-t border-white/5">
      <div className="max-w-7xl mx-auto">
        <div className="mb-20 text-left">
          <h2 className="text-spinecho-accent font-mono text-sm uppercase tracking-[0.2em] mb-4">Operational Capability</h2>
          <h3 className="text-3xl lg:text-6xl font-bold text-white max-w-4xl leading-tight tracking-tight">Engineered for high fidelity <br /> measurement integrity.</h3>
        </div>
          ...
          <div className="lg:col-span-5 space-y-3">
            {capabilities.map((item, index) => (
              <button 
                ...
                className={`w-full text-left p-6 rounded-xl border transition-all duration-500 flex items-center justify-between ${
                  activeIndex === index ? 'bg-spinecho-accent/10 border-spinecho-accent/40' : 'bg-white/[0.02] border-white/5 hover:border-white/10'
                }`}
              >
          ...
          <div className="lg:col-span-7">
            <AnimatePresence mode="wait">
              <motion.div 
                ...
                exit={{ opacity: 0, y: -10 }}
                className="glass-panel p-10 lg:p-16 border-white/5 bg-[#0F1115] text-left h-full flex flex-col"
              >
                <div className="mb-10 p-5 rounded-2xl bg-spinecho-accent/10 border border-spinecho-accent/20 w-fit text-spinecho-accent">
                ...
                <div className="pt-8 border-t border-white/10">
                  <span className="text-[10px] font-mono text-spinecho-accent uppercase tracking-widest block mb-2 font-bold">Commercial Impact</span>
                  <p className="text-white text-base font-semibold italic">{capabilities[activeIndex].impact}</p>
                </div>
              </motion.div>
            </AnimatePresence>
          </div>
