...
export default function EvolutionModal({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/95 backdrop-blur-xl p-4 md:p-6">
      <button 
        onClick={onClose} 
        className="absolute top-6 right-6 lg:top-10 lg:right-10 text-spinecho-slate hover:text-white font-mono text-[10px] tracking-widest uppercase z-50 bg-white/5 px-4 py-2 rounded border border-white/10 flex items-center gap-2"
      >
        Exit Briefing <X size={14} />
      </button>

      <div className="max-w-5xl w-full relative">
        <button onClick={prev} className="absolute -left-4 lg:-left-20 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full border border-white/10 flex items-center justify-center text-white z-50 bg-black/40 transition-all"><ChevronLeft size={24} /></button>
        <button onClick={next} className="absolute -right-4 lg:-right-20 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full border border-white/10 flex items-center justify-center text-white z-50 bg-black/40 transition-all"><ChevronRight size={24} /></button>
        ...
        <div className="glass-panel overflow-hidden bg-[#0A0C10] border-white/10 shadow-2xl min-h-[500px] flex flex-col p-8 lg:p-16">
          <div className="mb-12 text-left">
            <span className="text-spinecho-accent font-mono text-[10px] uppercase tracking-[0.4em] block mb-4">
              Architectural Evolution // {comparisons[current].topic}
            </span>
            <h2 className="text-3xl lg:text-4xl font-bold text-white uppercase tracking-tight leading-none">
              Next Generation NMR
            </h2>
            <p className="text-spinecho-slate mt-6 text-sm italic">Architectural refinements informed by field lessons from first generation NMR deployments.</p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12 text-left">
            <div className="p-8 rounded-xl bg-white/[0.02] border border-white/5">
              ...
              <p className="text-spinecho-slate text-sm leading-relaxed italic">{comparisons[current].legacy}</p>
            </div>
            <div className="p-8 rounded-xl bg-spinecho-accent/5 border border-spinecho-accent/20 relative overflow-hidden">
              <div className="absolute top-4 right-4 text-spinecho-accent opacity-20"><Box size={40} /></div>
              <span className="text-[9px] font-mono text-spinecho-accent uppercase tracking-widest block mb-4">Spin Echo Architecture</span>
              <p className="text-white text-base lg:text-lg leading-relaxed font-medium">{comparisons[current].se}</p>
            </div>
          </div>

          <div className="mt-auto pt-8 border-t border-white/5 text-left">
            <div className="flex flex-col lg:flex-row lg:items-center gap-2 lg:gap-4">
              <span className="text-[10px] font-mono text-spinecho-accent uppercase tracking-widest font-bold">Operational Implication:</span>
              <p className="text-white text-sm italic">{comparisons[current].impact}</p>
            </div>
          </div>
          <div className="flex gap-2 mt-12">{comparisons.map((_, i) => (<button key={i} onClick={() => setCurrent(i)} className={`h-1 transition-all duration-500 rounded-full ${i === current ? 'w-12 bg-spinecho-accent' : 'w-4 bg-white/10'}`} />))}</div>
        </div>
      </div>
    </div>
  );
