...
export default function UseCasePage() {
  ...
  const useCases = [
    { title: "Multiphase Measurement", tag: "Asset Optimization", challenge: "Operators require precise visibility into oil, gas, water, and condensate behavior.", whyHard: "Traditional measurement often relies on test separators that lose accuracy when fluid regimes shift.", approach: "Spin Echo utilizes physics based NMR sensing to identify molecular signatures.", value: "Reduces reliance on bulky test separators and manual sampling.", icon: <Activity size={20} /> },
    { title: "Wet Gas Monitoring", tag: "Infrastructure Protection", challenge: "Maintaining measurement certainty in high GVF environments.", whyHard: "Standard wet gas meters often struggle with changing liquid fractions.", approach: "High temporal response allow for stable measurement during slugging.", value: "Provides early detection of regime changes and protects downstream equipment.", icon: <Wind size={20} /> },
    { title: "NGL & Condensate Flows", tag: "Revenue Protection", challenge: "Capturing the commercial value of NGL requires live insight into fluid composition.", whyHard: "Compositional changes are traditionally tracked via periodic sampling.", approach: "Spin Echo provides in pipe measurement of valuable liquids.", value: "Enables production optimization based on actual liquid value.", icon: <Target size={20} /> },
    { title: "Carbon Capture (CCS)", tag: "Integrity Management", challenge: "CO2 transport pipelines require rigorous monitoring to prevent moisture ingress.", whyHard: "Existing sensors may lack the sensitivity to detect trace water in dense phase CO2.", approach: "Spin Echo senses the unique signature of water molecules without disturbing the flow.", value: "Enables an immediate operational response to maintain safety.", icon: <ShieldAlert size={20} /> },
    { title: "Hydrogen Blending", tag: "Energy Transition", challenge: "Hydrogen blending introduces variability in gas composition.", whyHard: "Turbines are highly sensitive to composition fluctuations.", approach: "Spin Echo provides real time composition measurement for mixed gas streams.", value: "Protects critical downstream assets from damage.", icon: <Microscope size={20} /> }
  ];
               ...
               <div className="bg-[#0A0C10] rounded-xl flex flex-col h-full overflow-hidden">
                 ...
                 <div className="p-8 lg:p-12 flex flex-col flex-grow text-left">
                    ...
                    <div className="mb-10 p-6 bg-spinecho-accent/5 border border-spinecho-accent/20 rounded-lg">
                      ...
                      <p className="text-white text-sm leading-relaxed mb-4 text-left">Replace legacy tank separation units with high fidelity NMR.</p>
                      <button onClick={() => setIsPartnerModalOpen(true)} className="text-spinecho-accent font-bold text-[10px] uppercase tracking-widest hover:underline block">Partner with us &rarr;</button>
                    </div>
                    <button onClick={openModal} className="w-full mt-auto py-4 bg-spinecho-accent text-white font-bold uppercase text-[10px] tracking-[0.3em] rounded hover:bg-blue-600 transition-all shadow-xl shadow-spinecho-accent/10">Book Pilot Campaign</button>
                 </div>
               </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-stretch">
            <div className="lg:col-span-5 space-y-4 text-left">
              {useCases.map((uc, index) => (
                <button key={index} onClick={() => setActiveIndex(index)} className={`w-full text-left p-6 lg:p-8 rounded-xl border transition-all duration-500 flex flex-col ${activeIndex === index ? 'bg-spinecho-accent/10 border-spinecho-accent/40' : 'bg-white/[0.02] border-white/5 hover:border-white/20'}`}>
